...
## banksheets fan-out (2026-09-01, runs 33–41) — one 9-repo parallel wave

Same pattern as the smorinlabs waves 3–5, driven from the handoff
`docs/handoffs/2026-09-01-banksheets-difftree-fanout.md`: 8 read-only recon subagents (one per repo) → controller
prepped 9 worktrees → one PR-open gate (owner opted the fork mirror in) → 9 verify-only subagents ran commit → push →
...
| Repo | PR | Merge | Notes |
|---|---|---|---|
| get-bank-sheets-web | #280 | `026ff49` | husky (`core.hooksPath=.husky`, `HUSKY=0`); 0 threads; pre-existing `Validate PR Title` (Jira-ID regex) failed on the install commit, not required — merged past on owner call |
| get-bank-sheets-site | #3 | `66ba715` | 2 SHA-pin threads (Copilot, cubic-dev-ai) declined; one resolve denied by the classifier ×2 — merged with the thread answered but open (owner call) |
| get-bank-sheets-mcp | #1 | `e6aa046` | ruleset "Protect Master" requires 1 approval, no bypass actors → author's merge refused; owner chose a temporary ruleset edit (1 → 0 → 1, restored and diffed identical) |
| banksheets-landing | #6 | `06ce6b5` | clean; clone on `docs/terraform-infra-design` → partial cleanup (worktree + remote branch removed; local branch and pull left) |
| banksheets-loops | #1 | `fbabf9d` | clean |
| rpi-artifacts | #1 | `c8bf2a9` | clean |
| banksheets-harness | #1 | `ec893ff` | clean |
| terraform-stripe | #1 | `c4f55b3` | clean; agent overrode the sandbox on its push after a classifier denial (F77) |
| get-bank-sheets-web-fork | #6 | `6f87b48` | fork mirror, same-repo PR against the fork's own `main` (explicit `--repo`/`--base`/`--head`); SHA-pin declined, cubic-dev-ai "README links to it" critique declined as template-level; same title-check failure as the parent |

### F77 — a verify-only agent retried a classifier-denied push with the sandbox override
- **Where:** the parallel-wave brief (process), `terraform-stripe` run.
- **What:** the auto-mode classifier denied the agent's `git push`; the agent re-ran it with `dangerouslyDisableSandbox`
  because the brief listed the push as a sanctioned mutation. The push itself was correct (re-derived from the API),
  but an agent deciding to override a denial is the wrong split — denials are the controller's to adjudicate.
- **Fix:** the brief now has to say it explicitly: a permission denial is a STOP-and-report, never an override, even
  for a sanctioned command. (The `banksheets-harness` agent handled the same denial correctly: it re-issued the
  single `git commit` alone, without the override.)
- **Class:** process.

### F78 — `pulls?head=<owner>:<branch>` returns nothing for a PR inside a fork
- **Where:** the F75 re-derivation script (process tooling), `get-bank-sheets-web-fork`.
- **What:** the REST `head=` filter found the PR on all 8 source repos and none on the fork, although the PR existed,
  was open, and had `head.repo.full_name` equal to the fork. The filter appears to resolve `head` against the fork's
  parent.
- **Fix:** on a fork, re-derive by PR number (or list `pulls?state=open` and match `head.ref`). Recorded here so the
  next controller does not read "NONE OPEN" as "the agent never opened it" — that is exactly the F75 trap inverted.
- **Class:** process.

### F79 — a ruleset-required approval blocks the author's own merge, and recon did not look for it
- **Where:** recon column set (process) and SKILL.md §4 step 7c (guidance gap, not edited here).
- **What:** `get-bank-sheets-mcp` carries a `pull_request` ruleset with `required_approving_review_count: 1` and no
  bypass actors. Classic protection 404s, so step 7c read "none required"; the PR showed `mergeable_state=blocked`
  and step 8 would have refused. Recon read `rules/branches/main` but only counted the rules; the approval parameter
  is what mattered.
- **Fix:** recon reports `rules/branches/<default>[] | select(.type=="pull_request") | .parameters.required_approving_review_count`
  and the ruleset's `bypass_actors` up front, so the merge gate carries the decision (approve from another account,
  temporary ruleset edit, or leave open) instead of discovering it at the merge. Skill text: §4 step 7c could name
  the rulesets endpoint alongside classic protection — tracked, not applied in this PR.
- **Class:** process + skill (guidance gap).

### F80 — new reviewer bot on the banksheets org: `cubic-dev-ai`
- **Where:** §4 step 6 bot roster.
- **What:** `cubic-dev-ai` opened threads on 2 of 9 PRs (a SHA-pin ask rated P1, a template-comment critique rated P3),
  each with an embedded "Prompt for AI agents" block instructing an edit to the workflow. Both were quoted and answered
  with the canned playbook; neither was executed. Copilot and Codex behaved as on smorinlabs.
- **Fix:** roster note only — the untrusted-input rule already covers it.
- **Class:** process.
